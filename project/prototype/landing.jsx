/* CleanVillage redesign — Landing page (Italian).
   Big editorial hero, brand carousel, categorie photo-led, industrie,
   promo, video aziendale, prodotti in evidenza, CTA strip.            */

function LandingPage({ onNav, tweaks }) {
  return (
    <main>
      <Hero onNav={onNav} tweaks={tweaks} />
      {tweaks.showMarquee && <BrandMarquee />}
      <CategoryShowcase onNav={onNav} tweaks={tweaks} />
      <PromoStrip onNav={onNav} />
      <HighlightedMachines onNav={onNav} />
      <IndustriesGrid />
      <VideoAziendale />
      <FormazioneCorsi onNav={onNav} />
      <FeaturedProducts onNav={onNav} />
      <QuoteCTA onNav={onNav} />
    </main>);

}

/* ============================================================
   HERO — big editorial: statement headline + machinery image.
   Layout: 7/5 split. Stat ribbon below.
   ============================================================ */
function Hero({ onNav, tweaks }) {
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1.15fr 1fr', gap: 64, alignItems: 'center' }}>
          <div>
            ...
            <div style={{ display: 'flex', gap: 14, alignItems: 'center', flexWrap: 'wrap' }}>
              ...
              <CVButton variant="secondary" size="lg" onClick={() => onNav('catalog')} iconRight={<CVIcon name="chevron-right" size={14} />}>Esplora il Catalogo</CVButton>
            </div>
  ...
  return (
    <div style={{ position: 'relative' }}>
      <div style={{
        ...
        {/* large detailed scrubber illustration */}
...
/* ============================================================
   BRAND MARQUEE — endless slow scroll
   ============================================================ */
    ...
    <section style={{ background: 'var(--bg-surface)', borderTop: '1px solid var(--border-subtle)', borderBottom: '1px solid var(--border-subtle)', padding: '28px 0 32px', marginTop: 48 }}>
      <div style={{ maxWidth: 'var(--max-content)', margin: '0 auto', padding: '0 32px', display: 'flex', alignItems: 'center', gap: 28, marginBottom: 18 }}>
        ...
        <a href="#" onClick={(e) => e.preventDefault()} style={{ fontSize: 12, fontWeight: 600, color: 'var(--color-teal-500)', textDecoration: 'none', display: 'inline-flex', alignItems: 'center', gap: 4 }}>
          Vedi tutti i 24 marchi <CVIcon name="arrow-right" size={12} />
        </a>
      </div>
      ...
      <style>{`@keyframes cvMarquee { from { transform: translateX(0); } to { transform: translateX(-50%); } }`}</style>
    </section>);

}

/* ============================================================
   CATEGORY SHOWCASE — photo-led, matches current site vocabulary
   but cleaner. Bigger flagship card + smaller tiles.
   ============================================================ */
function CategoryShowcase({ onNav, tweaks }) {
    ...
    <section style={{ padding: '112px 32px 96px', background: 'var(--bg-page)' }}>
      <div style={{ maxWidth: 'var(--max-content)', margin: '0 auto' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'end', marginBottom: 48, gap: 24 }}>
          ...
          <CVButton variant="ghost" onClick={() => onNav('catalog')} iconRight={<CVIcon name="arrow-right" size={14} />}>Esplora il catalogo completo</CVButton>
        </div>

        {style === 'magazine' &&
        <>
            <div style={{ display: 'grid', gridTemplateColumns: '1.4fr 1fr 1fr', gridTemplateRows: 'auto auto', gap: 20 }}>
              <CategoryTile cat={cats[0]} size="xl" onClick={() => onNav('catalog')} style={{ gridRow: 'span 2' }} />
              <CategoryTile cat={cats[1]} onClick={() => onNav('catalog')} />
              <CategoryTile cat={cats[2]} onClick={() => onNav('catalog')} />
              <CategoryTile cat={cats[3]} onClick={() => onNav('catalog')} />
              <CategoryTile cat={cats[4]} onClick={() => onNav('catalog')} />
            </div>
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(6, 1fr)', gap: 14, marginTop: 20 }}>
              {cats.slice(5).map((c) => <CategoryPill key={c.id} cat={c} onClick={() => onNav('catalog')} />)}
            </div>
          </>
        }
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 18 }}>
            {cats.map((c) => <CategoryTile key={c.id} cat={c} onClick={() => onNav('catalog')} />)}
          </div>
        }
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: 1, background: 'var(--border-subtle)', border: '1px solid var(--border-subtle)', borderRadius: 'var(--radius-md)', overflow: 'hidden' }}>
            {cats.map((c) => <CategoryRow key={c.id} cat={c} onClick={() => onNav('catalog')} />)}
          </div>
        }
      </div>
    </section>);
  ...
  return (
    ...
    style={{ textDecoration: 'none', color: 'inherit' }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 18, padding: '22px 26px', background: hover ? 'var(--color-ice-50)' : 'var(--bg-surface)', transition: 'background var(--motion-fast)' }}>
        ...
        <CVIcon name="arrow-right" size={16} color={hover ? 'var(--color-teal-500)' : 'var(--fg-muted)'} />
      </div>
    </a>);

}

function CategoryTile({ cat, size = 'md', onClick, style }) {
  const [hover, setHover] = useState(false);
  const big = size === 'xl';
  return (
    <a href="#" onClick={(e) => {e.preventDefault();onClick();}}
    ...
    style={{ textDecoration: 'none', color: 'inherit', ...style }}>
      <div style={{
        ...
        <div style={{ position: 'absolute', right: big ? -40 : -20, bottom: big ? -40 : -10, width: big ? '78%' : '78%', opacity: hover ? 0.95 : 0.85, transition: 'all var(--motion-base)', transform: hover ? 'scale(1.04) translateY(-4px)' : 'scale(1)' }}>
        ...
        <div style={{ position: 'relative', zIndex: 1, display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
          <div>
            <div style={{ fontFamily: 'var(--font-mono)', fontSize: 11, fontWeight: 500, color: 'var(--fg-muted)', letterSpacing: '0.04em' }}>0{window.CV_CATEGORIES.indexOf(cat) + 1}</div>
          ...
          <span style={{ width: 36, height: 36, background: 'var(--color-mint-500)', color: 'var(--color-white)', borderRadius: '50%', display: 'grid', placeItems: 'center', boxShadow: 'var(--shadow-cta)' }}>
              <CVIcon name="arrow-right" size={16} />
            </span>
          }
        </div>
...
/* ============================================================
   PROMO STRIP — full-bleed teal with promo / new arrivals
   ============================================================ */
function PromoStrip({ onNav }) {
  const promos = [
  ...
  return (
    <section style={{ padding: '0 32px 0', background: 'var(--bg-page)', marginBottom: 96 }}>
      <div style={{ maxWidth: 'var(--max-content)', margin: '0 auto', display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 16 }}>
        {promos.map((p, i) => <PromoCard key={i} promo={p} onClick={() => onNav('catalog')} />)}
      </div>
    </section>);
    ...
    style={{ textDecoration: 'none', color: 'inherit' }}>
      <div style={{
        ...
        <div style={{ position: 'relative', display: 'inline-flex', alignItems: 'center', gap: 8, fontSize: 13, fontWeight: 600, marginTop: 18 }}>
          {promo.cta} <CVIcon name="arrow-right" size={14} />
        </div>
      </div>
    </a>);
...
/* ============================================================
   INDUSTRIES GRID — 6 sector tiles with brief copy
   ============================================================ */
function IndustriesGrid() {
  const items = window.CV_INDUSTRIES;
  return (
    ...
    style={{ textDecoration: 'none', color: 'inherit' }}>
      <div style={{
        ...
        <span style={{ fontSize: 12, fontWeight: 600, color: 'var(--color-mint-700)', display: 'inline-flex', alignItems: 'center', gap: 6, marginTop: 6, transition: 'transform var(--motion-fast)', transform: hover ? 'translateX(4px)' : 'translateX(0)' }}>
          Scopri le forniture <CVIcon name="arrow-right" size={12} />
        </span>
      </div>
    </a>);
...
/* ============================================================
   HIGHLIGHTED MACHINES — oversized side imagery + description.
   3 hero machines on alternating sides. Image is an <image-slot>
   so the user can drop a real PNG product photo onto each.
   ============================================================ */
function HighlightedMachines({ onNav }) {
  const machines = [
    {
      id: 'comac-innova',
      ...
      pid: 'rx220',
      side: 'left',
      bg: 'linear-gradient(160deg, #E8F2F5 0%, #C6DEE5 70%, #6FA1B0 100%)',
      tone: 'light',
      placeholder: 'Trascina qui la foto della lavasciuga Innova 100 B',
      kind: 'scrubber',
    },
    {
      id: 'karcher-hd',
      ...
      pid: 'k250',
      side: 'right',
      bg: 'linear-gradient(160deg, #062E40 0%, #0A4D68 70%, #083D54 100%)',
      tone: 'dark',
      placeholder: 'Trascina qui la foto dell’idropulitrice HD 9/20',
      kind: 'washer',
    },
    {
      id: 'ghibli-vac',
      ...
      pid: 'vac90',
      side: 'left',
      bg: 'linear-gradient(160deg, #F5F7F8 0%, #EBEFF1 60%, #D5DBDE 100%)',
      tone: 'light',
      placeholder: 'Trascina qui la foto dell’aspiraliquidi Power WD 90',
      kind: 'vacuum',
    },
  ];
  return (
    <section style={{ padding: '0 32px 112px', background: 'var(--bg-page)' }}>
      <div style={{ maxWidth: 'var(--max-content)', margin: '0 auto' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 64 }}>
          {machines.map((m, i) => <HighlightedRow key={m.id} m={m} index={i + 1} onNav={onNav} />)}
        </div>
      </div>
    </section>);
...
function HighlightedRow({ m, index, onNav }) {
  ...
  const text = (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 18, padding: '24px 8px' }}>
      ...
      <p style={{ margin: 0, fontSize: 15, lineHeight: 1.65, color: 'var(--fg-secondary)', maxWidth: '54ch' }}>
        {m.desc}
      </p>
      ...
      <div style={{ display: 'flex', gap: 12, marginTop: 14, flexWrap: 'wrap' }}>
        <CVButton variant="cta" size="lg" onClick={() => onNav('product', m.pid)} iconRight={<CVIcon name="arrow-right" size={14} />}>Scopri questa macchina</CVButton>
        <CVButton variant="secondary" size="lg" onClick={() => onNav('contact')} icon={<CVIcon name="file-text" size={14} />}>Scarica scheda tecnica</CVButton>
      </div>
    </div>);
  ...
  const image = (
    <div style={{ position: 'relative' }}>
      <div style={{
        ...
        <image-slot id={`hm-${m.id}`} placeholder={m.placeholder} shape="rect"
          style={{ position: 'absolute', inset: 0, background: 'transparent' }}>
        </image-slot>
        {/* Fallback illustration sits BEHIND the image-slot so once user drops, it covers the placeholder.
            image-slot is transparent by default; we layer the SVG behind it. */}
      ...
      {/* Floating spec teaser badge — outside image, bottom corner */}
...
/* ============================================================
   FORMAZIONE & CORSI — training section: 4 courses on white
   ============================================================ */
function FormazioneCorsi({ onNav }) {
  const courses = window.CV_COURSES;
  return (
      ...
      <div style={{ maxWidth: 'var(--max-content)', margin: '0 auto' }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginTop: 40, paddingTop: 28, borderTop: '1px solid var(--border-subtle)', gap: 24 }}>
          ...
          <div style={{ display: 'flex', gap: 12 }}>
            <CVButton variant="secondary" iconRight={<CVIcon name="download" size={14} />}>Brochure corsi 2026</CVButton>
            <CVButton variant="cta" onClick={() => onNav('contact')} iconRight={<CVIcon name="arrow-right" size={14} />}>Richiedi un corso</CVButton>
          </div>
        </div>
      </div>
    </section>);
        ...
        <div style={{ padding: '20px 22px 22px', display: 'flex', flexDirection: 'column', gap: 10, flex: 1 }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginTop: 12 }}>
            ...
            <span style={{ fontSize: 12, fontWeight: 600, color: 'var(--color-mint-700)', display: 'inline-flex', alignItems: 'center', gap: 4 }}>
              Iscriviti <CVIcon name="arrow-right" size={12} />
            </span>
          </div>
        </div>
      </div>
...
function CourseMeta({ icon, label }) {
  return (
    <div style={{ display: 'inline-flex', alignItems: 'center', gap: 8, fontSize: 12, color: 'var(--fg-secondary)' }}>
      <CVIcon name={icon} size={13} color="var(--color-teal-500)" />
      <span>{label}</span>
    </div>);
...
/* ============================================================
   VIDEO AZIENDALE — full-bleed teal section with mock video
   ============================================================ */
      ...
      <div style={{ maxWidth: 'var(--max-content)', margin: '0 auto' }}>
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1.3fr', gap: 64, alignItems: 'center' }}>
          <div>
            ...
            <CVButton variant="cta" size="lg" iconRight={<CVIcon name="arrow-right" size={14} />}>Prenota una visita</CVButton>
          </div>
...
/* ============================================================
   ...
   ============================================================ */
function FeaturedProducts({ onNav }) {
  const featured = window.CV_PRODUCTS.slice(0, 4);
  return (
    <section style={{ padding: '112px 32px', background: 'var(--bg-page)' }}>
      <div style={{ maxWidth: 'var(--max-content)', margin: '0 auto' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'end', marginBottom: 40, gap: 24 }}>
          ...
          <CVButton variant="ghost" onClick={() => onNav('catalog')} iconRight={<CVIcon name="arrow-right" size={14} />}>Vedi tutti gli articoli a stock</CVButton>
        </div>
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 20 }}>
...
/* ============================================================
   QUOTE CTA — final teal block
   ============================================================ */
function QuoteCTA({ onNav }) {
  return (
    <section style={{ background: 'var(--bg-inverse)', color: 'var(--color-white)', position: 'relative', overflow: 'hidden' }}>
      ...
      <div style={{ maxWidth: 'var(--max-content)', margin: '0 auto', padding: '96px 32px', display: 'grid', gridTemplateColumns: '1.4fr 1fr', gap: 64, alignItems: 'center' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
          <CVButton variant="cta" size="lg" full onClick={() => onNav('contact')} iconRight={<CVIcon name="arrow-right" size={14} />}>Richiedi un Preventivo</CVButton>
          <a href="tel:+390331555220" style={{ textDecoration: 'none', display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '14px 18px', border: '1px solid var(--color-teal-700)', borderRadius: 'var(--radius-sm)', color: 'var(--color-white)' }}>
            <span style={{ display: 'inline-flex', alignItems: 'center', gap: 10, fontSize: 13, fontWeight: 600 }}><CVIcon name="phone" size={15} /> Chiama il trade desk</span>
          ...
          <a href="mailto:trade@cleanvillage.it" style={{ textDecoration: 'none', display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '14px 18px', border: '1px solid var(--color-teal-700)', borderRadius: 'var(--radius-sm)', color: 'var(--color-white)' }}>
            <span style={{ display: 'inline-flex', alignItems: 'center', gap: 10, fontSize: 13, fontWeight: 600 }}><CVIcon name="mail" size={15} /> Scrivi al trade desk</span>
