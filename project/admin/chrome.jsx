/* CleanVillage Admin — shared chrome (sidebar, topbar, table primitives).
   The admin uses the same tokens as the storefront but a tighter density,
   monochrome chrome, and a left sidebar instead of a top mega-menu.        */
...
function AdminShell({ page, onNav, children }) {
  return (
    <div style={{display:'grid', gridTemplateColumns:'248px 1fr', minHeight:'100vh', background:'var(--bg-page)'}}>
      <AdminSidebar page={page} onNav={onNav}/>
...
function AdminSidebar({ page, onNav }) {
  const groups = [
    { label: 'Generale', items: [
      { id:'dashboard', icon:'layout-dashboard', label:'Dashboard' },
      { id:'orders',    icon:'shopping-bag',     label:'Ordini & RDA',  badge: 12 },
      { id:'quotes',    icon:'file-text',        label:'Preventivi',    badge: 4 },
    ]},
    { label: 'Catalogo', items: [
      { id:'products',  icon:'package',          label:'Prodotti',      count: 571 },
      { id:'categories',icon:'folder-tree',      label:'Categorie',     count: 11  },
      { id:'brands',    icon:'tag',              label:'Marchi',        count: 24  },
      { id:'pricing',   icon:'percent',          label:'Listini & sconti' },
    ]},
    { label: 'Contenuti landing', items: [
      { id:'videos',    icon:'video',            label:'Video' },
      { id:'highlights',icon:'star',             label:'Macchine in evidenza' },
      { id:'promos',    icon:'megaphone',        label:'Promozioni' },
      { id:'courses',   icon:'graduation-cap',   label:'Corsi & formazione' },
    ]},
    { label: 'Amministrazione', items: [
      { id:'users',     icon:'users',            label:'Buyer trade' },
            ...
            {g.items.map(it => {
              ...
              return (
                <a key={it.id} href="#" onClick={(e)=>{e.preventDefault(); onNav(it.id);}}
                  ...
                  {it.badge != null && (
                    <span style={{fontFamily:'var(--font-mono)', fontSize:10, fontWeight:600, padding:'1px 6px', borderRadius:999, background: active ? 'var(--color-mint-500)' : 'rgba(255,255,255,0.18)', color: active ? 'var(--color-white)' : 'var(--color-white)'}}>{it.badge}</span>
                  )}
  ...
  return (
    <header style={{height:62, background:'var(--bg-surface)', borderBottom:'1px solid var(--border-subtle)', padding:'0 28px', display:'flex', alignItems:'center', gap:20, flexShrink:0, position:'sticky', top:0, zIndex:5}}>
      <div style={{display:'flex', alignItems:'center', gap:12, fontFamily:'var(--font-mono)', fontSize:11, color:'var(--fg-muted)'}}>
        ...
        <span>admin</span>
        <AdminIcon name="chevron-right" size={11}/>
        <span style={{color:'var(--fg-primary)', fontWeight:600}}>{titles[page]}</span>
      </div>
      <span style={{flex:1}}/>
      ...
      <a href="../Cleanvillage.it Redesign.html" target="_blank" style={{display:'inline-flex', alignItems:'center', gap:6, padding:'8px 13px', fontSize:12, fontWeight:600, color:'var(--color-teal-500)', textDecoration:'none', border:'1px solid var(--border-strong)', borderRadius:'var(--radius-sm)'}}>
...
/* ============================================================
   ICON helper (admin scope — re-uses lucide via inline render)
   ============================================================ */
function AdminIcon({ name, size = 16, color, strokeWidth = 1.75, style }) {
  const ref = useRef();
  useEffect(() => {
...
/* ============================================================
   PAGE WRAPPER — title block + actions
   ============================================================ */
function AdminPage({ eyebrow, title, subtitle, actions, children }) {
  return (
...
/* ============================================================
   ...
   ============================================================ */
function ABtn({ children, variant='primary', size='md', icon, iconRight, onClick, type='button', disabled }) {
...
function Panel({ title, action, children, padding='22px 24px' }) {
  return (
...
/* ============================================================
   MODAL / DRAWER
   ============================================================ */
function Modal({ open, onClose, title, width=720, children, footer }) {
  if (!open) return null;
  return (
...
function AField({ label, required, hint, span=1, children }) {
  return (
...
function AInput(p) { return <input {...p} style={{...adminInputStyle, ...(p.style||{})}}/>; }
function ATextarea(p) { return <textarea rows={3} {...p} style={{...adminInputStyle, resize:'vertical', minHeight:80, ...(p.style||{})}}/>; }
function ASelect(p) { return <select {...p} style={{...adminInputStyle, ...(p.style||{})}}>{p.children}</select>; }
...
Object.assign(window, {
  AdminShell, AdminPage, AdminIcon, ABtn, Badge, Panel, Modal,
  AField, AInput, ATextarea, ASelect, adminInputStyle, DataTable,
});
