/* Admin · Landing videos manager + Landing courses manager */

function AdminVideos({ onNav }) {
  const [items, setItems] = useState(window.CV_LANDING_VIDEOS);
        ...
        <FormSect title="Posizione sul sito" step={2}>
          ...
          {form.spot === 'Pagina prodotto' && (
            <AField label="Prodotto associato" hint="Cerca per nome o SKU">
              <ASelect>
                {window.CV_PRODUCTS.map(p => <option key={p.id} value={p.id}>{p.brand} · {p.name} ({p.sku})</option>)}
              </ASelect>
            </AField>
          )}
        </FormSect>
...
Object.assign(window, { AdminVideos });
